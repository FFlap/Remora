...
test.describe("Rich Text Format Parity", () => {
  test("keeps bold styling and font family consistent across quick preview and creative view", async ({ page }) => {
    ...
    const quickWeight = await quickPreviewCard.evaluate(
      (node, tokenValue) => {
    ...
    const quickFontFamily = await quickPreviewCard.evaluate((node) => window.getComputedStyle(node).fontFamily);
    ...
    const creativeWeight = await creativeInlineEditor.evaluate(
      (node, tokenValue) => {
        let maxWeight = Number.parseInt(window.getComputedStyle(node).fontWeight, 10) || 0;
        const elements = [node, ...Array.from(node.querySelectorAll("*"))];
        for (const element of elements) {
    ...
    const creativeFontFamily = await creativeInlineEditor.evaluate((node) => window.getComputedStyle(node).fontFamily);
