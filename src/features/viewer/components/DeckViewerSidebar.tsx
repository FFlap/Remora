...
type FilteredViewerSection = {
  section: ViewerSection;
  visibleCards: VisibleViewerCard[];
};

function filterViewerSections(
  sections: ViewerSection[],
  normalizedSearchValue: string,
): FilteredViewerSection[] {
  return sections
    .map((section) => {
...
function DeckViewerSidebarSection({
  section,
  visibleCards,
  selectedCardId,
  isCollapsed,
...
export function DeckViewerSidebar({
  ...
  const filteredSections = useMemo(
    () => filterViewerSections(sections, normalizedSearchValue),
        ...
        {filteredSections.length === 0 ? (
          ...
          <div className="space-y-2">
            {filteredSections.map(({ section, visibleCards }) => (
              <DeckViewerSidebarSection
                key={String(section._id)}
                section={section}
                visibleCards={visibleCards}
                selectedCardId={selectedCardId}
                isCollapsed={collapsedSections[String(section._id)] ?? false}
