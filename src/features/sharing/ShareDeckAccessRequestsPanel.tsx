...
export function ShareDeckAccessRequestsPanel({ deckId }: ShareDeckAccessRequestsPanelProps) {
  ...
  return (
    <div className="space-y-2 rounded-lg border border-border bg-muted/20 p-3">
      ...
      {accessRequests?.map((request) => {
        ...
        const isResolving = resolvingId === requestId;
        return (
          <div
            key={requestId}
          ...
          >
            ...
            <div className="flex gap-1">
              <Button
                size="sm"
                variant="outline"
                disabled={request.status === "approved" || isResolving}
                onClick={async () => {
              ...
              <Button
                size="sm"
                variant="ghost"
                disabled={request.status === "rejected" || isResolving}
                onClick={async () => {
                  if (isResolving) return;
                  setResolvingId(requestId);
                  try {
